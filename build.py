...
def saveScriptAndMeta(script,outDir,filename,oldDir=None):
...
for fn in glob.glob("plugins/*.user.js"):
    ...
    downloadUrl = distUrlBase and distUrlBase + '/' + os.path.basename(fn).replace("\\","/") or 'none'
    ...
    saveScriptAndMeta(script, outDir, os.path.basename(fn), oldDir)
